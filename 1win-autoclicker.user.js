...
// @author       mudachyo
// @match        https://clicker-frontend.tma.top/*
// @grant        none
// @icon         https://clicker-frontend.tma.top/images/common/Coin.webp
...
(function () {
    ...
    if (window.Telegram && window.Telegram.WebApp) {
        if (typeof window.Telegram.WebApp.disableVerticalSwipes !== 'function') {
            window.Telegram.WebApp.disableVerticalSwipes = () => {
                console.log('1win Autoclicker: Функция disableVerticalSwipes вызвана (заглушка)');
            };
        }
    } else {
        console.warn('1win Autoclicker: window.Telegram.WebApp не найден');
    }
    ...
    function autoclicker() {
        ...
        if (element) {
            ...
            console.log('1win Autoclicker: Элемент #coin не найден, повторная попытка через 1 секунду');
    ...
    console.log('1win Autoclicker: Автокликер будет запущен через 5 секунд');
    setTimeout(() => {
        autoclicker();
    }, 5000);
})();